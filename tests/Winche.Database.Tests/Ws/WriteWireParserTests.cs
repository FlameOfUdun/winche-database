...
public class WriteWireParserTests
{
    private static IReadOnlyList<Write> Parse(string json) =>
        WriteWireParser.Parse((JsonArray)JsonNode.Parse(json)!);

    private static RuntimeException Throws(string json)
    {
        var ex = Assert.Throws<RuntimeException>(() => Parse(json));
    ...
    {
        // the parser accepts it; WriteValidator (runtime) rejects non-merge set sentinels
    ...
    {
        // exists must be a bool; a string value is INVALID_ARGUMENT
    ...
    {
        // updateTime must be a string; a number is INVALID_ARGUMENT
