...
public class DocumentSqlTests
{
    ...
    {
        var result = DocumentSql.Get("documents", "users/u1");
    ...
    {
        var result = DocumentSql.Upsert("documents", "users/u1", "u1", "users", """{"a":{"integerValue":"1"}}""");
    ...
    {
        var result = DocumentSql.DeleteSubtree("documents", "users/100%_x");
    ...
    {
        var result = DocumentSql.UpdateData("documents", "users/u1", "{}");
        Assert.Equal(2, result.Parameters.Length);
    ...
    {
        Assert.EndsWith("FOR UPDATE", DocumentSql.Get("documents", "users/u1", forUpdate: true).Sql);
        Assert.DoesNotContain("FOR UPDATE", DocumentSql.Get("documents", "users/u1").Sql);
    }
}
