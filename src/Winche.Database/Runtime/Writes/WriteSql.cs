...
internal static class WriteSql
{
    ...
    /// </summary>
    internal static CompiledSql LockAll(string table, IReadOnlyList<string> exactPaths, IReadOnlyList<string> cascadeRoots)
    {
        ...
        var sql = $"SELECT {Columns} FROM {table} WHERE {string.Join(" OR ", clauses)} ORDER BY path FOR UPDATE";
    ...
    /// </summary>
    internal static CompiledSql Upsert(string table, string path, string id, string collection,
        string dataJson, DateTimeOffset createdAt, DateTimeOffset updatedAt, long version, long versionBefore)
    {
        ...
        var sql = $"""
            ...
            WHERE {table}.version = {bag.Add(versionBefore)}
            """;
        return new CompiledSql(sql, bag.ToArray());
    }

    internal static CompiledSql DeletePaths(string table, IReadOnlyList<string> paths)
    {
        var bag = new ParameterBag();
        return new CompiledSql($"DELETE FROM {table} WHERE path = ANY({bag.Add(paths.ToArray())})", bag.ToArray());
    }

    internal static CompiledSql InsertChanges(string table,
        ...
        IReadOnlyList<long> versions, DateTimeOffset commitTime)
    {
        var bag = new ParameterBag();
        var sql = $"""
            INSERT INTO {table}_changes (type, path, collection, version, commit_time)
