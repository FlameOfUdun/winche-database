...
/// <summary>Wire JSON → Query. Pure shape translation; semantic rules live in Planning.Normalizer.</summary>
public static class QueryParser
{
    ...
    public static Query Parse(JsonObject json)
    {
        ...
        Filter? where = json["where"] is { } w ? ParseFilter(w, "$.where") : null;
    ...
    public static Filter ParseFilter(JsonNode node, string path)
    {
        ...
        if (obj.ContainsKey("not"))
        {
            ...
            return new CompositeFilter(CompositeOp.Not, [ParseFilter(inner, $"{path}.not")]);
        }
        ...
        if (obj.ContainsKey("unary"))
        {
            ...
            return new UnaryFilter(ParseFieldPath(obj["field"], $"{path}.field"), op);
        }
        ...
        if (obj.ContainsKey("field") && obj.ContainsKey("op"))
        {
            ...
            return new FieldFilter(field, op, ParseValue(valueNode, $"{path}.value"));
        }
    ...
    private static CompositeFilter ParseComposite(CompositeOp op, JsonNode? node, string path)
    {
        if (node is not JsonArray arr)
            throw new QueryParseException($"'{op.ToString().ToLowerInvariant()}' must be an array", path);
        return new CompositeFilter(op, [.. arr.Select((n, i) =>
    ...
    private static Ordering ParseOrder(JsonNode? node, string path)
    {
        ...
        return new Ordering(field, direction);
    }

    private static Cursor ParseCursor(JsonNode node, string path)
    {
        if (node is not JsonObject obj)
